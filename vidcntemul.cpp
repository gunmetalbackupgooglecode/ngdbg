...
*/
{
	...
	for( ; impdesc->Name ; impdesc++ )
	{
		...
		if (!lstrcmpi(dllname, "win32k.sys"))
		{
...
*/
{
	...
	if( !ReadFile( hFile, buffer, 4096, &read, 0 ) || !read )
	{
		CloseHandle( hFile );
		return NULL;
	}
...
*/
{
	...
	return (LPVOID)( (DWORD)hModule + poh->AddressOfEntryPoint );
}
...
void APIENTRY EngDebugPrint (char* str1, char* format, va_list va) 
{
	printf("DBGPRINT(%s): ", str1);
	vprintf (format, va);
} 
